...
def loss_generator(which=None, norm_loss_=None):
    ...
    if (which == "Strong") or (which == "default") or (which == "Vanilla"):
        ...
        def loss_fun(diff_model, static_model, input, out, idx, beta=1.0, find_weight=None, **kwargs):
            ...
            return find_weighted_loss([norm_loss_(pred, out), loss_contr], weight_vals=wv), (norm_loss_(pred, out), loss_contr)
...
def get_data(problem, folder=None, google=True, **kwargs):
    ...
    match problem:
        ...
        case "fruits":
            ...
            if os.path.exists(f"{folder}/fruits_data.npy"):
                all_data = np.load(f"{folder}/fruits_data.npy")
            else:
                data_res = 64
                celeb_transform = lambda im: np.astype(
                    resize(im, (data_res, data_res, 3), order=1, anti_aliasing=True)
                ...
                def read_jpgs_to_array(folder):
                    images = []
                    for filename in os.listdir(folder):
                        if filename.endswith(".jpg"):
                            img = Image.open(os.path.join(folder, filename))
                            img_array = np.array(img)
                            img_array = celeb_transform(img_array)
                            images.append(img_array)
                    return np.stack(images)

                all_data = []
                for fold in tqdm(os.listdir(folder)):
                    all_data.append(read_jpgs_to_array(os.path.join(folder, fold)))
                all_data = np.concatenate(all_data, axis=0)
                all_data = jnp.swapaxes(all_data, 0, -1)
                np.save(f"{folder}/fruits_data.npy", all_data)
            pre_func_in = lambda x: jnp.astype(x, jnp.float32) / 255.0
            pre_func_out = lambda x: jnp.astype(x, jnp.float32) / 255.0
            return all_data, None, None, None, all_data, None, pre_func_in, pre_func_out, ()
...
class MLP_with_CNNs_trans(eqx.Module, strict=True):
    ...
    def __init__(
        ...
        *,
        ...
        #     width = inp
        # else:
        #     assert width > inp, "width of MLP should not be smaller than latent space"
        assert CNNs_num == len(width_CNNs)
