...
import pytest
from AE_classes import Strong_RRAE_MLPs, Vanilla_AE_MLP, Weak_RRAE_MLPs, Strong_RRAE_CNN, Weak_RRAE_CNN, Vanilla_AE_CNN
...
class Test_AEs_shapes:
    ...
    def test_Weak_MLP(self, latent, num_modes, dim_D):
        ...
        model = Weak_RRAE_MLPs(
            x, latent, num_modes, key=jrandom.PRNGKey(0)
        )
        y = model.encode(x)
        assert y.shape == (latent, dim_D)
        x = model.decode(y)
    ...
    def test_Weak_CNN(self, latent, num_modes, dim_D):
        x = jrandom.normal(jrandom.PRNGKey(0), (500, 500, dim_D))
        model = Weak_RRAE_CNN(
            x, latent, num_modes, dim_D, key=jrandom.PRNGKey(0)
        )
        y = model.encode(x)
        assert y.shape == (latent, dim_D)
        x = model.decode(y)
...
class Test_MLPs_width():
    def test_MLP(self, width, depth):
        ...
        model = Strong_RRAE_MLPs(x, 200, 3, width_enc=width, depth_enc=depth, key=jrandom.PRNGKey(0))
        try:
            model(x)
        except:
            raise ValueError("Width is not correctly specified")
        assert all([model._encode.mlp.layers[i].weight.shape[0] == true_width[i] for i in range(depth)])

        model = Weak_RRAE_MLPs(x, 200, 3, width_dec=width, depth_dec=depth, key=jrandom.PRNGKey(0))
        try:
            model(x)
        except:
            raise ValueError("Width is not correctly specified")
        assert all([model._decode.mlp.layers[i].weight.shape[0] == true_width[i] for i in range(depth)])