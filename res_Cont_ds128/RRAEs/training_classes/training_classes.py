    ...
    def fit(
        ...
        *,
        ...
        def make_step(model, input, out, opt_state, idx, sharding, **loss_kwargs):
            ...
            return loss, model, opt_state, aux, old_model, grads
        ...
        for steps, lr, batch_size in zip(step_st, lr_st, batch_size_st):
            try:
                ...
                for step, (inp_b, out_b, idx) in zip(
                    range(steps),
                    dataloader(
                    ...
                    out_b = self.model.norm_out(pre_func_out(out_b)).T
                    inp_b = inp_b.T
                    inp_b = pre_func_inp(inp_b)
                    inp_b, out_b = eqx.filter_shard((inp_b, out_b), sharding)
                    loss, model, opt_state, aux, old_model, grads = make_step(
                        model,
                        inp_b,
                        out_b,
                        opt_state,
                    ...
                    if ((step % save_every) == 0) or jnp.isnan(loss):
                        if jnp.isnan(loss):
                            print(f"Step for nan {step}")
                            import pdb

                            pdb.set_trace()
                            self.model = last_10[-2]
                        else:
                            self.model = model
                        orig = (
                            f"checkpoint_{step}"
...
class RRAE_Trainor_class(Trainor_class):
    def fit(self, *args, training_key, **kwargs):
        ...
        if ("pre_func_inp" in  training_kwargs) or ("pre_func_inp" in kwargs):
            if "pre_func_inp" in  training_kwargs:
                pre_func_inp = training_kwargs["pre_func_inp"]
            else:
