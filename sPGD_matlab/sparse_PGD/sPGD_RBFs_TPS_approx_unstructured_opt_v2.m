function [fP,d_fP,dd_fP,MOR,Error] = sPGD_RBFs_TPS_approx_unstructured_opt_v2(data,np_vec,param_vec,param_vec_lim,index_int,error_crit,Ncp,h,coef_reg,flag_reg,flag_modes)
...
%%%%   thin_plate_spline BASIS FUNCTIONS PGD APPROX unstructured data  %%%%
    ...
    if i<=index_int
        ...
        if 0
            ...
            if ~new
                ...
                Sf_r{i} = sqrt((cp{i}^2)*( x_cp{i}' - x_sp{i} ).^2 + 1); 
                %}
            else
                cp{i} = 1*ones(1,N{i});
                cp{i}(1) = 1;
                cp{i}(end) = 1;
                r = sqrt(( x_cp{i}' - x_sp{i} ).^2);
                e = cp{i}';
                Sf_r{i} = thin_plate_spline(e,r); 
                %Sf_r{i} = sqrt((cp{i}'.^2).*( x_cp{i}' - x_sp{i} ).^2 + 1); 
            end
        end
        ...
        if ~new
            ...
            r = sqrt(( x_cp{i}' - pi{i} ).^2);
            e = cp{i}';
            termsp{i} = thin_plate_spline(e,r);
