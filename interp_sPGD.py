...
import numpy as np
import jax
from train_RRAE import my_vmap, make_model
import jax.random as jrandom    
import matplotlib.pyplot as plt
from train_alpha_nn import load_eqx_nn

def interp_spgd_in_matlab(datas, parameters):
    ...
    for i in range(datas.shape[0]):
        offline_name = f"offline_python_{i}"
        v_n = datas[i:i+1, :]
        std = jnp.std(v_n)
        mean = jnp.mean(v_n)
        data = (v_n - mean) / std
        ...
        print("Saved")
        pred_train = jnp.stack(my_vmap(lambda p: jnp.asarray(eng.eval_MOR(eng.load(str_MOR), md(p))))(parameters_train)).T
        pred_test = jnp.stack(my_vmap(lambda p: jnp.asarray(eng.eval_MOR(eng.load(str_MOR), md(p))))(parameters_test)).T
...
if __name__ == "__main__":
    ...
    interp_spgd_in_matlab(vt_train, p_vals)
    pdb.set_trace()